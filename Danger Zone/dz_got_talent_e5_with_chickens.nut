/////////////////////////////////////////////////////////////////////////////////////////////////////
// Purpose: This script was used to practice for the fifth episode of the Bump Mine Competition
//          "Danger Zone's Got Talent": https://youtube.com/playlist?list=PLyCGb0pwEr_SHF2ef6XJBpvQUfpY_oaXe
//          This script is identical to dz_got_talent_e5.nut, except a feature to reset chickens to
//          their previous position is added.
// 
...
//   In CSGO, load the correct map by entering this into the console: game_mode 0;game_type 6;map dz_vineyard
//   To start and reset the practice, enter this into the console: script_execute dz_got_talent_e5_with_chickens
//
// Made by lacyyy:
...
DEFAULT_PLAYER_START_POS <- Vector(5146.103516, -375.833832, 736.031250)
DEFAULT_PLAYER_START_ANGLES <- Vector(3.677797, 48.477852, 0.000000)


BOT_POSITION <- Vector(5917.338379, -288.337158, 1224.031250)
...
const LOGIC_RELAY_NAME = "dzgt_ep5_relay"
...
function dzgt_e5_get_player() { // human player, no bot
	local t_player = null
...
function dzgt_e5_get_point_servercommand() {
...
function setstart() { // Called by user with:   script setstart()
	...
	if(player) {
		...
		dzgt_e5_reset() // Reset to remember the equipped weapons
	} else {
...
function rememberchickens() {
	CHICKEN_POSITIONS = []
	local chicken = null
	while(chicken = Entities.FindByClassname(chicken, "chicken")) {
		CHICKEN_POSITIONS.append(chicken.GetOrigin())
	}
	local msg = ""
	if(CHICKEN_POSITIONS.len() > 0) {
		msg += "\x01 \x04 Chicken positions have been remembered! Chickens will return to these on each reset!"
	} else {
		msg += "\x01 \x02 Could not find any chicken on the map! Chicken resetting is now disabled."
		msg += " You can create chicken by looking on the ground and entering this into the console:\n"
		msg += "\x05 ent_create chicken"
		
	}
	ScriptPrintMessageChatAll(msg)
}

function Rememberchickens() { rememberchickens() }
function RememberChickens() { rememberchickens() }
function rememberChickens() { rememberchickens() }
function REMEMBERCHICKENS() { rememberchickens() }

function dzgt_e5_loop_tick()
{
...
function dzgt_e5_create_relay()
{
	...
	relay.GetScriptScope().OnTrigger <- dzgt_e5_loop_tick // Add a reference to the function
...
function dzgt_e5_on_player_death()
{
	if(!activator) return
	...
	if(activator == player)
	{
		...
		EntFireByHandle(player, "RunScriptFile", "dz_got_talent_e5_with_chickens", PLAYER_RESPAWN_DELAY, null, null)
		
		// If player died because of script initialization, equip him once he respawned
		if(DZGT_E5_FIRST_INIT) {
			dzgt_e5_run_server_command("give weapon_bumpmine", PLAYER_RESPAWN_DELAY + 0.1)
			// Give knife later so player first picks up the knife they had before, if they had one
			dzgt_e5_run_server_command("give weapon_knife_butterfly", PLAYER_RESPAWN_DELAY + 0.6)
		}
...
function dzgt_e5_create_playerdeath_listener()
{
	...
	ent.GetScriptScope().OnUse <- dzgt_e5_on_player_death // Add a reference to the function
	ent.ConnectOutput( "OnUse", "OnUse" ) // On each OnUse output, execute the function
}

function dzgt_e5_is_initialized() {
	return Entities.FindByName(null, LOGIC_RELAY_NAME) != null
}

function dzgt_e5_init()
{
	...
	dzgt_e5_run_server_commands(commands)
	...
	dzgt_e5_create_relay()
	dzgt_e5_create_playerdeath_listener()
	dzgt_e5_print_help()
}

function dzgt_e5_print_equip_help()
{
...
function dzgt_e5_reset()
{
	...
	if(player)
	{
		...
		if(IS_PLAYER_DEAD) // If this is the first reset after the player's death and respawn
		{
			if(PLAYER_RESPAWN_WITH_EXOJUMP) {
				dzgt_e5_run_server_command("exojump")
			}
		...
		player.SetHealth(120)
		dzgt_e5_run_server_command("noclip 0")
	}
	...
	// Chicken resetting is disabled if CHICKEN_POSITIONS array is empty (default)
	if(CHICKEN_POSITIONS.len() > 0) {
		local chicken = null
		foreach(pos in CHICKEN_POSITIONS) {
			if(chicken = Entities.FindByClassname(chicken, "chicken")) {
				chicken.__KeyValueFromVector("origin", pos)
			} else {
				ScriptPrintMessageChatAll("\x01 \x08| \x01 Some chickens went missing \x02:( \x01since you last saved")
				ScriptPrintMessageChatAll("\x01 \x08| \x01 their positions with\x05 script rememberchickens()\x05")
				ScriptPrintMessageChatAll("\x01 \x08| \x01 They will keep missing until you add more with:")
				ScriptPrintMessageChatAll("\x01 \x08| \x05 ent_create chicken")
				break
			}
		}
	}
	...
	dzgt_e5_print_equip_help()
	
	dzgt_e5_run_server_command("r_cleardecals") // Only happens for hosting player
}

function dzgt_e5_check_game_settings()
{
	local illegalMap = GetMapName().find("dz_vineyard") == null
	...
	if(illegalMap)
	{
		ScriptPrintMessageChatAll("\x01 \x09 This practice script was made to be played on dz_vineyard, not "
			+ GetMapName())
		ScriptPrintMessageCenterAll("This script won't work, it was made for dz_vineyard!")
	}
	if(illegalGamemode)
	{
		...
		ScriptPrintMessageChatAll("\x01 \x09 game_mode 0;game_type 6;map dz_vineyard")
...
function dzgt_e5_print_help()
{
	...
	ScriptPrintMessageChatAll("\x01 \x04| \x01 To reset chickens to their current position on each reset,")
	ScriptPrintMessageChatAll("\x01 \x04| \x01 enter the following into the console:")
	ScriptPrintMessageChatAll("\x01 \x04| \x05 script rememberchickens()")
	ScriptPrintMessageChatAll("\x01 \x04| \x01 You can create chickens by looking on the ground and")
	ScriptPrintMessageChatAll("\x01 \x04| \x01 entering the following into the console:")
	ScriptPrintMessageChatAll("\x01 \x04| \x05 ent_create chicken")
}
...
if(ScriptIsWarmupPeriod())
{
	...
	if(!dzgt_e5_is_initialized()) {
		DZGT_E5_FIRST_INIT <- true
		dzgt_e5_init()
		
		// Kill player to remove parachute and exojump
		dzgt_e5_run_server_command("noclip 0")
		dzgt_e5_run_server_command("kill", 0.1) // Kill with delay to make sure the player death listener is initialized
		// Player gets equipped with exojump, knife, bumpmines once he respawns
	} else {
		DZGT_E5_FIRST_INIT <- false
		dzgt_e5_reset()
	}
}
else
{
	dzgt_e5_run_server_command("mp_warmup_start")
	dzgt_e5_run_server_command("mp_warmup_pausetimer 1")
	...
	ScriptPrintMessageChatAll("\x01 \x02| \x04 game_mode 0;game_type 6;map dz_vineyard")
}
// Check if we're on the correct map, gamemode and tickrate
dzgt_e5_check_game_settings()
