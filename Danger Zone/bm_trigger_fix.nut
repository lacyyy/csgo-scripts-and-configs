...
::bmtf_create_relay <- function()
{
	...
	if (!has_old_relay_existed) {
		...
		relay.__KeyValueFromString( "targetname", BMTF_LOGIC_RELAY_NAME )
		...
		local targetParams = BMTF_LOGIC_RELAY_NAME + ":Trigger::" + tickLength + ":-1"	
		EntFireByHandle( relay, "AddOutput", "OnTrigger " + targetParams, 0, null, null )
	}
	
	relay.ValidateScriptScope()
	relay.GetScriptScope().OnTrigger <- bmtf_loop_tick // Add a reference to the function
...
::bmtf_init <- function() {
	...
	ScriptPrintMessageChatAll("\x01 this functionality again. Just run the script again with:")
	ScriptPrintMessageChatAll("\x04 script_execute bm_trigger_fix")
}
